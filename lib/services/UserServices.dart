...
class UserServices {
  ...
  static Future<String> getToken(String username, String password) async {
    ...
    http.Response response = await http.post(
        "$protocol://$ip:$port/Microblog/api/login",
        headers: {"Content-Type": "application/json"},
        body: bodyJson);
    if (response.headers.containsKey("authorization")) {
      token = response.headers["authorization"];
      u = await getUser(
          "$protocol://$ip:$port/Microblog/api/users/search/findByUsername?username=$username");
  ...
  static Future<String> getUsers() async {
    http.Response response =
        await http.get('$protocol://$ip:$port/Microblog/api/users');
  ...
  static Future<String> getPosts() async {
    http.Response response = await http
        .get('$protocol://$ip:$port/Microblog/api/posts?size=$pageSize');
    String body = utf8codec.decode(response.bodyBytes);
    return body;
  }
  ...
  static Future<String> getPost(int id) async {
    http.Response response = await http
        .get('$protocol://$ip:$port/Microblog/api/posts/${id.toString()}');
    String body = utf8codec.decode(response.bodyBytes);
    return body;
  }
  ...
  static Future<String> getComments() async {
    http.Response response =
        await http.get('$protocol://$ip:$port/Microblog/api/comments');
    String body = utf8codec.decode(response.bodyBytes);
    return body;
  }
  ...
  static void createPost(Map postMap) async {
    ...
    http.Response response = await http.post(
        '$protocol://$ip:$port/Microblog/api/posts',
  ...
  static void createComment(Map commentMap) async {
    ...
    http.Response response = await http.post(
        '$protocol://$ip:$port/Microblog/api/comments',
        headers: {"Content-Type": "application/json", "authorization": token},
        body: body);
  }
  ...
  static void createUser(Map userMap) async {
    ...
    http.Response response = await http.post(
        '$protocol://$ip:$port/Microblog/api/users',
        headers: {"Content-Type": "application/json"},
        body: body);
  }
  ...
  static void deletePost(String id) async {
    http.Response response = await http.delete(
        '$protocol://$ip:$port/Microblog/api/posts/$id',
  ...
  static Future<List> getCommentsByPostId(String id) async {
    http.Response response = await http.get(
        "$protocol://$ip:$port/Microblog/api/comments/search/findCommentsByPost_Id?id=$id");
    String body = utf8codec.decode(response.bodyBytes);
